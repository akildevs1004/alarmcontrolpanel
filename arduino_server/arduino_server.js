    ...
    while (boundary > -1) {
      ...
      try {
        ...
        if (dataObject.type != "heartbeat")
          console.log("Received config from device:", serialNumber, dataObject);
...
app.get("/device-config/:serialNumber", (req, res) => {
  ...
  if (deviceData) {
    ...
    } else res.status(404).json({ error: "Socket not found" });
    res.json({
      serialNumber: serialNumber,
...
app.post("/device-config-update/:serialNumber", (req, res) => {
  ...
  if (deviceData) {
    ...
    if (deviceData.socket && !deviceData.socket.destroyed) {
      ...
      console.log("socket Device not found or not connected");
