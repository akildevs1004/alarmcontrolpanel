<template>
  <v-app>
    <div class="text-center ma-2">
      <v-snackbar :timeout="2000" v-model="snackbar" top="top" elevation="24">
        {{ response }}
      </v-snackbar>
    </div>
    <v-navigation-drawer
    ...
    >
      ...
      <!-- <v-btn
        v-if="getLoginType == 'security' || getLoginType == 'branch'"
        icon
        plan
        @click="goToSettings()"
        class="mr-3"
  ...
  methods: {
    ...
    setTopmenuHilighter() {
      const routeMap = {
        ...
        "alarm-reports": { name: "alarm_reports", path: "/security/reports" },
        "alarm-customers": {
