      ...
      <v-col>
        <v-card elevation="0" class="mt-2">
          <v-toolbar class="mb-2 white--text" color="white" dense flat>
            <v-toolbar-title>
              <span style="color: black"> Technicians</span></v-toolbar-title
            >
...
<script>
import EditTechnician from "../../components/Alarm/EditTechnician.vue";
  ...
  methods: {
    ...
    deleteItem(item) {
      if (confirm("Are you sure want to delete  ?")) {
        ...
        this.$axios.delete(`technicians/${item.id}`).then(({ data }) => {
          this.snackbar = true;
