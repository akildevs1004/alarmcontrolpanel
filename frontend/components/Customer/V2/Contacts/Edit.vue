<template>
  <v-dialog v-model="contactCreateDialog" width="760">
    ...
    <template v-slot:activator="{ on, attrs }">
      <span>
        <v-icon v-bind="attrs" v-on="on" color="primary" small>
          mdi-pencil
        </v-icon>
      </span>
    </template>
    ...
    <v-card>
      <v-alert dense flat> Edit Contact </v-alert>
      <v-container>
        <v-row>
          <v-col cols="3" dense>
          ...
          <v-col cols="12" class="text-right">
            <v-btn small :loading="loading" color="primary" @click="update">
              Submit
            </v-btn></v-col
          >
        </v-row>
...
export default {
  ...
  methods: {
    ...
    async update() {
      ...
      for (const key in this.payload_primary) {
        if (
          ...
          contact.append(key, this.payload_primary[key]);
      }
      contact.append("company_id", this.$auth.user.company_id);
      contact.append("customer_id", this.contact.customer_id);
      if (this.contact?.id) contact.append("editId", this.contact.id);

      if (this.primary_upload.name) {
        contact.append("profile_picture", this.primary_upload.name);
      }
    ...
    closeResponsePopup() {
      this.$emit("callrefreshDataFromChild", this.response);
