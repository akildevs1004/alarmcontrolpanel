    ...
    <v-dialog v-model="dialogEventsList" max-width="80%">
      <v-card>
        <v-card-title dark class="popup_background_noviolet">
          <span style="color: black" dense>Monthly Statistics</span>
          <v-spacer></v-spacer>
          <v-icon
            style="color: black"
            @click="
...
export default {
  ...
  methods: {
    ...
    loadDevicesStatistics() {
      ...
      this.$axios
        .get(`/technician_tickets_monthly_stats`, options)
        .then(({ data }) => {
          this.data = data;
    ...
    async renderChart1(data) {
      ...
      this.chartOptions.labels[0] = "Pending";
      this.chartOptions.series[0] = data.pending_count ?? 0;

      this.chartOptions.labels[1] = "Closed";
      this.chartOptions.series[1] = data.resolved_count ?? 0;
