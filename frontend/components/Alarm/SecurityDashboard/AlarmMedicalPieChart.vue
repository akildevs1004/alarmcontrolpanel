    ...
    <v-dialog v-model="dialogEventsList" max-width="80%">
      <v-card>
        <v-card-title dark class="popup_background_noviolet">
          <span dense style="color: black">Medical Events</span>
          <v-spacer></v-spacer>
          <v-icon
            style="color: black"
        ...
        <v-card-text style="padding: 20px; padding-left: 0px">
          <v-card class="elevation-2">
            <v-card-text class="mt-5">
              <AlamAllEvents
                name="medicalPieChart"
                style="padding: 0px; padding-top: 0px"
                :key="key"
                :popup="true"
                :eventFilter="'Medical'" /></v-card-text
          ></v-card>
        </v-card-text>
      </v-card>
    </v-dialog>
    <v-row style="margin-top: -27px"
      ><v-col cols="8" style="color: black; font-size: 12px">Medical</v-col>
